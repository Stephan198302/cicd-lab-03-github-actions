...
**6. `project.json` — `scripts/validate.sh` (malformed JSON).** `seed.sh` leaves a **trailing
...
`.yamllint.yml` comment extended. Every linter silent and `scripts/validate.sh` exits 0.
